...
interface RoadmapClientProps {
    ...
    initialReleases: (Release & { roadmap_features: RoadmapFeature[] })[];
}

export default function RoadmapClient({ initialRoadmap, initialReleases }: RoadmapClientProps) {
    const router = useRouter();
    const [roadmap, setRoadmap] = useState(initialRoadmap);
    const [releases, setReleases] = useState(initialReleases);
    const [view, setView] = useState<'ROADMAP' | 'RELEASES'>('ROADMAP');
    ...
    const handleDeleteRelease = async (id: string) => {
        if (confirm('DELETE RELEASE? THIS ACTION CANNOT BE UNDONE. FEATURES WILL NOT BE DELETED.')) {
        ...
        <div className="w-full max-w-7xl mx-auto p-4 animate-fadeIn">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b-2 border-border-normal pb-6 gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    {view === 'ROADMAP' ? (
                        ...
                        <Button variant="secondary" className="text-xs" onClick={handleOpenCreateRelease}>
                             + NEW RELEASE
                        </Button>
                    )}
                </div>
            ...
            {view === 'ROADMAP' ? (
                            ...
                            >
                                <div className="flex items-center gap-6 relative z-10 w-full md:w-auto">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-[10px] text-gray-500 uppercase flex gap-4 mt-1 font-mono tracking-wider">
                                            ...
                                            {item.release_id && <span className="text-purple-400">// RELEASED</span>}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-2 mt-4 md:mt-0 relative z-10 md:opacity-0 group-hover:opacity-100 transition-opacity justify-end w-full md:w-auto border-t border-border-normal pt-4 md:border-none md:pt-0">
                                    {item.status !== 'completed' && (
                                        <button
                                            onClick={() => handleCompleteFeature(item)}
                ...
                <div className="grid gap-4">
                        ...
                        <div key={release.id} className="bg-bg-secondary border border-border-normal p-6 group hover:border-border-strong transition-colors relative overflow-hidden">
                             <div className="flex flex-col md:flex-row justify-between items-start gap-6 relative z-10">
                                <div className="flex-1">
                                    ...
                                    {/* Associated Features */}
                                    <div className="space-y-1 pl-4 border-l border-gray-800">
                                        {release.roadmap_features && release.roadmap_features.length > 0 ? (
                                            release.roadmap_features.map(feat => (
                                                <div key={feat.id} className="text-[10px] font-mono text-gray-400 uppercase flex gap-2">
                                                    <span className="text-secondary">-</span> {feat.title}
                                                </div>
                                            ))
                                        ) : (
                                            <div className="text-[10px] font-mono text-gray-600 italic">NO FEATURES LINKED</div>
                                        )}
                                    </div>
                                </div>
                     ...
                     {releases.length === 0 && (
                        <div className="p-12 text-center text-gray-500 font-mono text-xs border border-dashed border-gray-800 uppercase tracking-widest">
                            // NO RELEASES FOUND. INITIATE FIRST VERSION.
                        </div>
                    )}
                </div>
            )}
