...
export const ReviewCard: React.FC<ReviewCardProps> = ({ review }) => {
  return (
    <Link href={`/reviews/${review.id}`} className="block group">
      <div className="relative border border-border-subtle bg-bg-primary/40 overflow-hidden h-full flex flex-col hover:border-cyan-500/50 hover:shadow-[0_0_25px_-5px_rgba(34,211,238,0.2)] transition-all duration-300">
        ...
        <div className="relative h-48 w-full overflow-hidden border-b border-border-subtle">
           ...
           {/* Score Badge */}
           <div className="absolute top-4 right-4 bg-bg-primary/80 backdrop-blur border border-cyan-500/50 px-3 py-1 flex items-center gap-2 shadow-lg">
        ...
        <div className="p-6 flex-1 flex flex-col">
            <div className="mb-4">
                ...
                <h3 className="font-bold text-xl text-text-primary group-hover:text-cyan-400 transition-colors leading-tight mb-2 font-sans tracking-tight">
                    {review.title}
                </h3>
            </div>
