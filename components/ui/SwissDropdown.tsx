...
export function SwissDropdown<T extends string | number>({
    ...
    return (
        <div className={`relative inline-block ${className}`} ref={wrapperRef}>
                ...
                className={`flex items-center justify-between w-full gap-2 font-mono uppercase tracking-wider border transition-all
                    ...
                    ${inverted
                        ? (isOpen
                            ...
                            : 'text-text-primary border-border-normal hover:border-text-primary bg-bg-secondary/40')
                        : (isOpen
                            ? 'bg-bg-tertiary text-text-primary border-text-primary'
                            : 'text-text-secondary border-border-normal hover:border-text-primary bg-bg-secondary/50')
                    }
                    ${buttonClassName}
                `}
            >
            ...
            {isOpen && (
                <div className={`absolute top-[calc(100%+4px)] right-0 min-w-full w-max bg-bg-primary border border-border-normal shadow-2xl flex flex-col ${menuClassName}`}>
                    {options.map((option) => (
                        <button
