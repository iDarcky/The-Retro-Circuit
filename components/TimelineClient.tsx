...
const TimelineClient: FC<TimelineClientProps> = ({ events }) => {
  const [selectedEvent, setSelectedEvent] = useState<TimelineEvent | null>(events.length > 0 ? events[0] : null);

  if (events.length === 0) {
      return (
          <div className="text-center font-mono text-gray-500 py-20 border-2 border-dashed border-gray-800">
