...
const MetricCell = ({ label, value, unit }: { label: string, value: string | number | undefined, unit?: string }) => {
    return (
        <div className="flex flex-col gap-1 p-4 border border-border-subtle hover:border-orange-500/50 transition-colors bg-bg-tertiary">
            <span className="text-[10px] font-mono text-gray-500 uppercase tracking-widest">{label}</span>
            <div className="font-pixel text-lg text-text-primary truncate">
                {value ? (
                    <>
                        {value}
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4 font-mono text-xs">
                ...
                {/* Category */}
                <div className="flex items-start justify-between border-b border-border-strong/5 pb-2">
                    <span className="text-gray-500">CATEGORY</span>
                    <span className="text-text-primary uppercase text-right">{category}</span>
                </div>

                {/* Form Factor */}
                <div className="flex items-start justify-between border-b border-border-strong/5 pb-2">
                    <span className="text-gray-500">FORM FACTOR</span>
                    <span className="text-text-primary uppercase text-right">{formFactor}</span>
                </div>

                 {/* OS */}
                <div className="flex items-start justify-between border-b border-border-strong/5 pb-2">
                    <span className="text-gray-500">OS</span>
                    <span className="text-text-primary uppercase text-right">{os}</span>
                </div>

                 {/* CPU */}
                <div className="flex items-start justify-between border-b border-border-strong/5 pb-2">
                    <span className="text-gray-500">CPU</span>
                    <span className="text-text-primary uppercase text-right">{cpu}</span>
                </div>

                 {/* RAM */}
                <div className="flex items-start justify-between border-b border-border-strong/5 pb-2">
                    <span className="text-gray-500">RAM</span>
                    <span className="text-text-primary uppercase text-right">{ram}</span>
                </div>
