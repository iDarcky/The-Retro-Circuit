        ...
        {games.length > 0 && (
            <div className="p-6 border-t border-retro-grid bg-retro-dark">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Link to="/archive" className="font-mono text-xs text-retro-blue hover:text-white">VIEW FULL ARCHIVE &gt;</Link>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {games.slice(0, 6).map(game => (
                        <Link to={`/archive/${game.slug || game.id}`} key={game.id} className="group block">
