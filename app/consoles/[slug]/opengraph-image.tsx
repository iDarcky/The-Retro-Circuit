...
// Force Edge Runtime for optimal image generation performance
...
export default async function Image({ params }: { params: { slug: string } }) {
  ...
  // Clean up image URL (ensure absolute if needed, but relative usually works if hosted locally)
  // However, satori needs absolute URLs or base64.
  // Assuming finalImage is a relative path like '/consoles/...' or absolute 'https://...'
  const isRelative = finalImage && finalImage.startsWith('/');
  const imageUrl = isRelative
    ? `https://theretrocircuit.com${finalImage}`
    : (finalImage || 'https://theretrocircuit.com/logo.png');
