...
SRC='/root/.claude/uploads/f59a1cad-1fd0-5d4f-a900-427f6214ea8d/f6b23aab-handhelds_v4.xlsx'
...
BRAND={'anbernic':'Anbernic','retroid / moorechip':'Retroid','ayaneo':'Ayaneo',
 ...
 '乐天游':'Lotiyo','gamepad digital':'GPD','jinxing digital':'JinXing Digital','powkiddy':'Powkiddy','trimui':'Trimui','miyoo / bittboy':'Miyoo','game kiddy':'Game Kiddy','waveshare':'Waveshare','hardkernel':'HardKernel','clockworkpi':'ClockworkPi','retroflag':'Retroflag','gamepark holdings':'GamePark Holdings','none':'Unknown','gamepad digital':'GPD','game pad digital':'GPD','ayaneo':'Ayaneo','ayn technologies':'Ayn','one netbook':'One Netbook','one netbook, tencent':'One Netbook','aokzoe (one netbook spinoff)':'AOKZOE','konkr (ayaneo)':'KONKR','asus & microsoft':'Asus','msi':'MSI','lenovo':'Lenovo','acer':'Acer'}
def brand_of(b):
    b=' '.join(str(b or '').split())
    return BRAND.get(b.lower(), b)

EXISTING=set(open('/tmp/claude-0/-home-user-The-Retro-Circuit/f59a1cad-1fd0-5d4f-a900-427f6214ea8d/scratchpad/existing_slugs.txt').read().strip().split(','))
# The live catalogue mixes slug conventions ("anbernic-rg-28xx" vs "anbernic-rg353p"),
# so compare on alphanumerics only or true duplicates slip through on hyphen placement.
def squash(s): return re.sub(r'[^a-z0-9]', '', str(s).lower())
EXISTING_SQUASHED={squash(x) for x in EXISTING}
try:
    _v3=json.load(open('v3_import.json'))
    EXISTING |= {o['slug'] for o in _v3}
    EXISTING_SQUASHED |= {squash(o['slug']) for o in _v3}
...
for r in range(2, ws.max_row+1):
    ...
    if slug in EXISTING or squash(slug) in EXISTING_SQUASHED:
        skipped.append(slug); continue
    out.append({'name':disp,'manufacturer':brand,'slug':slug,
                'form_factor':form_factor(cell(r,4)),'device_category':'pc_gaming',
                'status':'draft','variants':[v]})

json.dump(out, open('v4_import.json','w'), indent=1, ensure_ascii=False)
slugs=[o['slug'] for o in out]
print("already in DB, skipped:", len(skipped))
